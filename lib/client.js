...
async function get(config) {
  ...
  if (config.clientAssertionSigningKey) {
    fallbackClient.name = 'private_key';
    fallbackClient.client = new issuer.Client(
      {
    ...
    applyHttpOptionsCustom(fallbackClient.client);
    fallbackClient.client[custom.clock_tolerance] = config.clockTolerance;
    ...
    if (config.idpLogout) {
      if (
        ...
        Object.defineProperty(fallbackClient.client, 'endSessionUrl', {
          value(params) {
            const { id_token_hint, post_logout_redirect_uri, ...extraParams } =
              params;
            const parsedUrl = url.parse(urlJoin(issuer.issuer, '/v2/logout'));
            parsedUrl.query = {
              ...extraParams,
              returnTo: post_logout_redirect_uri,
              client_id: fallbackClient.client.client_id,
            };
  ...
  if (config.clientSecret) {
    defaultClient.name = 'client_secret';
    defaultClient.client = new issuer.Client(
      {
        client_id: config.clientID,
        client_secret: config.clientSecret,
        id_token_signed_response_alg: config.idTokenSigningAlg,
    ...
    applyHttpOptionsCustom(defaultClient.client);
    defaultClient.client[custom.clock_tolerance] = config.clockTolerance;
    ...
    if (config.idpLogout) {
      if (
        ...
        Object.defineProperty(defaultClient.client, 'endSessionUrl', {
          value(params) {
            const { id_token_hint, post_logout_redirect_uri, ...extraParams } =
              params;
            const parsedUrl = url.parse(urlJoin(issuer.issuer, '/v2/logout'));
            parsedUrl.query = {
              ...extraParams,
              returnTo: post_logout_redirect_uri,
              client_id: defaultClient.client.client_id,
            };
  ...
  return { clients: { defaultClient, fallbackClient }, issuer };
}
...
exports.set = (config, clients, issuer) => {
  const promise = new Promise((resolve) => resolve({ clients, issuer }))
  cache.set(config, promise);
  return promise;
}
