...
const { get: getClient, set: setClient } = require('./client');
...
async function refresh({ tokenEndpointParams } = {}) {
  ...
  const { clients: { defaultClient, fallbackClient } } = await getClient(config);
  ...
  try {
    newTokenSet = await defaultClient.client.refresh(oldTokenSet, extras);
  } catch (e) {
    console.log('error at refresssshhh:', e)
  }
...
class RequestContext {
  ...
  async fetchUserInfo() {
    ...
    const { clients: { defaultClient, fallbackClient } } = await getClient(config);
    return defaultClient.client.userinfo(tokenSet.call(this));
  }
}
...
class ResponseContext {
  ...
  async login(options = {}) {
    ...
    try {
      ...
      if (config.pushedAuthorizationRequests) {
        const { request_uri } = await defaultClient.client.pushedAuthorizationRequest(
          authParams
        );
      ...
      const authorizationUrl = defaultClient.client.authorizationUrl(authParams);
      debug('redirecting to %s', authorizationUrl);
      res.redirect(authorizationUrl);
    } catch (err) {
      console.log('error at logginnnn:', err);
      next(err);
    }
  }
  ...
  async logout(params = {}) {
    ...
    try {
      ...
      returnURL = defaultClient.client.endSessionUrl({
        ...config.logoutParams,
        id_token_hint,
  ...
  async callback(options = {}) {
    ...
    next = once(next);
    try {
      const { clients: { defaultClient, fallbackClient }, issuer } = await getClient(config);
      const redirectUri = options.redirectUri || this.getRedirectUri();

      let tokenSet;
      try {
        const callbackParams = defaultClient.client.callbackParams(req);
        ...
        tokenSet = await defaultClient.client.callback(redirectUri, callbackParams, checks, {
          exchangeBody: {
            ...(config && config.tokenEndpointParams),
            ...options.tokenEndpointParams,
          },
        });
        console.log(`success with default! ${defaultClient.name}`, tokenSet)
      } catch (error) {
        console.log(`error after attempting default ${defaultClient.name}:`, error);
        if (fallbackClient.client) {
          try {
            const callbackParams = fallbackClient.client.callbackParams(req);
            const authVerification = transient.getOnce(
              config.transactionCookie.name,
              req,
              res
            ...
            tokenSet = await fallbackClient.client.callback(redirectUri, callbackParams, checks, {
              exchangeBody: {
                ...(config && config.tokenEndpointParams),
                ...options.tokenEndpointParams,
              },
            });
            console.log(`success with fallback! ${fallbackClient.name}`, tokenSet)
            setClient(config, { defaultClient: fallbackClient, fallbackClient: defaultClient }, issuer);
          } catch (fallbackError) {
            console.log(`error after attempting fallback ${fallbackClient.name}:`, fallbackError);
            throw createError(400, fallbackError.message, {
              error: fallbackError.error,
              error_description: fallbackError.error_description,
            });
          }
        }
      }
